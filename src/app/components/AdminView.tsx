  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
        <div className="bg-slate-50 px-4 sm:px-6 py-4 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <h2 className="text-base sm:text-lg font-black text-slate-800 flex items-center gap-2"><PackagePlus size={20} className="text-emerald-600"/> Reabastecimiento de Materia Prima (Granel)</h2>
          <span className="text-[10px] sm:text-xs font-bold bg-emerald-100 text-emerald-700 px-2 py-1 rounded uppercase tracking-wider">Gestión de Stock</span>
        </div>
        <form onSubmit={reabastecerGranel} className="p-4 sm:p-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
        <div className="bg-slate-50 px-4 sm:px-6 py-4 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <h2 className="text-base sm:text-lg font-black text-slate-800 flex items-center gap-2"><DollarSign size={20} className="text-purple-600"/> Motor de Pricing Dinámico & Catálogo</h2>
          <span className="text-[10px] sm:text-xs font-bold bg-purple-100 text-purple-700 px-2 py-1 rounded uppercase tracking-wider">Edición de Costos Habilitada</span>
        </div>
        <div className="p-4 sm:p-6">
          ...
          <div className="grid gap-6">
            {granel.map((g: any) => (
              <div key={g.id} className="border-2 border-slate-100 rounded-lg p-4 sm:p-5">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                    <label className="text-xs sm:text-sm font-semibold text-slate-600">Costo de compra:</label>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold">$</span>
                      <input type="number" value={g.cost} onChange={(e) => setGranel((prev: any) => prev.map((item: any) => item.id === g.id ? {...item, cost: Number(e.target.value)} : item))} className="pl-7 pr-3 py-1.5 w-32 border-2 border-slate-300 rounded-md font-bold text-slate-800 outline-none focus:border-purple-500 text-sm" />
                    </div>
                  </div>
                </div>
                <div className="bg-slate-50 rounded p-3 sm:p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
        <div className="bg-slate-50 px-4 sm:px-6 py-4 border-b border-slate-200">
          <h2 className="text-base sm:text-lg font-black text-slate-800 flex items-center gap-2"><LayoutDashboard size={20} className="text-blue-600"/> Matriz de Inventario Multilocal</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm min-w-[500px]">
            <thead className="bg-slate-100 text-slate-600">
              <tr>
                <th className="p-3 sm:p-4 font-bold uppercase text-xs tracking-wider">SKU</th>
                <th className="p-3 sm:p-4 font-bold uppercase text-xs tracking-wider text-center border-l border-slate-200">Depósito Central</th>
                <th className="p-3 sm:p-4 font-bold uppercase text-xs tracking-wider text-center border-l border-slate-200">Sucursal Posadas 1</th>
                <th className="p-3 sm:p-4 font-bold uppercase text-xs tracking-wider text-right">Total Consolidado</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {granel.map((g: any) => (
                <tr key={g.id} className="bg-orange-50/30">
                  <td className="p-3 sm:p-4 font-semibold text-slate-800 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-orange-500"></span> {g.name}</td>
                  <td className="p-3 sm:p-4 text-center font-bold border-l border-slate-100 text-slate-700">{g.stock?.["Central"] || 0} kg</td>
                  <td className="p-3 sm:p-4 text-center font-bold border-l border-slate-100 text-slate-700">{g.stock?.["Posadas"] || 0} kg</td>
                  <td className="p-3 sm:p-4 text-right font-black text-orange-600">{(g.stock?.["Central"] || 0) + (g.stock?.["Posadas"] || 0)} kg</td>
                </tr>
              ))}
              {catalogoConPrecios.map((f: any) => (
                <tr key={f.id} className="hover:bg-slate-50">
                  <td className="p-3 sm:p-4 font-medium text-slate-700 flex items-center gap-2 pl-6 sm:pl-8"><span className="w-1.5 h-1.5 rounded-full bg-blue-400"></span> {f.name}</td>
                  <td className="p-3 sm:p-4 text-center font-semibold border-l border-slate-100">{f.stock?.["Central"] || 0} un.</td>
                  <td className="p-3 sm:p-4 text-center font-semibold border-l border-slate-100">{f.stock?.["Posadas"] || 0} un.</td>
                  <td className="p-3 sm:p-4 text-right font-bold text-blue-600">{(f.stock?.["Central"] || 0) + (f.stock?.["Posadas"] || 0)} un.</td>
                </tr>
              ))}
            </tbody>
          </table>
